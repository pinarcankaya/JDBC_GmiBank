...
public class US_06_Test {
    ...
    public void TC_0602() throws SQLException {
        String queryMin="select  min(event_id),name\n" +
                "from  jhi_persistent_audit_evt_data\n" +
                "group by name\n" +
                "order by name limit 1 ";
        List<Map<String,String>> nameList=DatabaseConnector.getQueryAsAListOfMaps(queryMin);
        System.out.println(nameList);
        Assert.assertEquals(nameList.get(0).get("min"),"10450");

    }

    //3---(hangi eyaletin toplam sayisi 10'dan fazla ise bulunuz)
    @Test
    public void TC_0603() throws SQLException, UnsupportedEncodingException {
